...
- Launch history reuses the visibility texture. It filters by launch date and leaves the source reference positions fixed; it does not animate docking or claim historically accurate placement. app/history.ts supplies sourced relocation notes.
...
Prebuilt output is committed. Regenerate with `python scripts/convert-station.py work/iss scripts/station.json work/restored` (numpy and Pillow), then `node scripts/publish-model.mjs work/restored`, then `python scripts/build-launches.py`. All source triangles must survive. The legacy optimizer deliberately rejects materials manifests. `scripts/lwo-materials.py` reads surfaces, image references and UV maps; missing texture files and unsupported legacy effects are recorded in the manifest. `npm test` validates ranges, attributes, textures and source triangle counts. When the audit's raw pre-simplification conversion is available, `python scripts/validate-source-geometry.py work/audit/raw-model public/models` independently checks exact oriented triangle equality for every part. See docs/source-appearance.md for scope and archive omissions.
...
This repository is the station fork created by `docs/iss-explorer-plan.md`'s phase commands (`/iss-1-convert` through `/iss-5-textures`), run against the original Human Atlas repository. That plan document is kept here for reference; the available source color textures and materials are now transferred, with archive omissions documented.
