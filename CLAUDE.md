...
This repository is the station fork created by `docs/iss-explorer-plan.md`'s phase commands (`/iss-1-convert` through `/iss-5-textures`), run against the original Human Atlas repository. That plan document is kept here for reference; phases 1-3 are complete.
