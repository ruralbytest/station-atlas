...
export async function decodeModelResponse(response:Response,expectedBytes:number,compressed:boolean):Promise<ArrayBuffer>{
 if(!response.ok)throw new Error('A station model file could not be loaded.');
 ...
 if(buffer.byteLength!==expectedBytes)throw new Error('A station model file was incomplete. Please reload the viewer.');
 return buffer;
}
